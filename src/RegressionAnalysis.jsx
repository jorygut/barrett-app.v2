...
function RegressionAnalysis() {
  ...
  const uploadData = async () => {
    ...
    try {
      ...
      const response = await fetch('https://barrett-api.onrender.com/regress', {
        method: 'POST',
        body: formData,
      });
      if (response.status === 200) {
        ...
        a.href = url;
        a.download = 'Regression.txt'; // Set the filename
  ...
  const uploadGraph = async () => {
    ...
    try {
      const response = await fetch('https://barrett-api.onrender.com/partreg', {
        method: 'GET',
      });
      if (response.status === 200) {
        alert('Files uploaded successfully');
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'partial_regression_plot.png'; // Set the filename
  ...
  const uploadRefFit= async () => {
  
    try {
      const response = await fetch('https://barrett-api.onrender.com/resfit', {
        method: 'GET',
      });
      if (response.status === 200) {
        alert('Files uploaded successfully');
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'residuals_vs_fitted.png'; // Set the filename
  ...
  return (
    <div>
      ...
      <div>
        <button className='graph-upload' onClick={uploadGraph}> Download Partial Regression Graph</button>
    </div>
    <div>
        <button className='res-fit-upload' onClick={uploadRefFit}> Download Residual vs Fitted Graph</button>
    </div>
    </div>
  );
}
